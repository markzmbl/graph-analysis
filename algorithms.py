...
def simple_temporal_cycles(G: nx.MultiDiGraph, length_bound=None, temporal_bound=None):
    ...
    Notes
    -----
    When length_bound is None, the current_timestamp complexity is $O((n+e)(c+1))$ for $n$
    nodes, $e$ edges and $c$ simple circuits.  Otherwise, when length_bound > 1,
    the current_timestamp complexity is $O((c+n)(k-1)d^k)$ where $d$ is the average degree of
